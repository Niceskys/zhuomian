...
Zhuomian 的产品方向成立，但 Desktop Hosting、前台窗口感知、键盘焦点、真实背景模糊和部署方式仍是未经验证的工程假设。项目当前只允许进入 **Phase 0：Specification & Feasibility**，不得把 WorkerW、真实 Acrylic 或任何单一安装方式描述为既定生产架构。
...
- 产品实现：尚未开始。
- Desktop Hosting、真实背景 Blur、Packaging：均未验证或决定。
- Phase 0：等待规范基线合并及仓库 Ruleset 配置。
