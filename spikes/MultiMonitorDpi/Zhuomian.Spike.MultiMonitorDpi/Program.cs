...
internal static class Program
{
    ...
    {
        NativeMethods.SetProcessDpiAwarenessContext(MultiMonitorDpiProbe.PerMonitorV2);

        var outputPath = GetOption(args, "--output");
        using var probe = new MultiMonitorDpiProbe();
        var evidence = probe.Run();
        var json = JsonSerializer.Serialize(
            evidence,
            MultiMonitorDpiJsonContext.Default.MultiMonitorDpiEvidence);
